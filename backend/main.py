...
from models import (
    GraphState, NodeItem, EdgeItem, NodeStatus, DependencyType, ComponentType,
    SimulationRequest, SimulationResponse, RecoveryRequest, RecoveryResponse
)
from simulator import run_cascade_simulation, run_recovery_simulation

app = FastAPI(
    title="BreakPoint API",
    description="System Failure & Cascade Simulation Engine",
    version="1.0.0"
)
...
DEFAULT_SCENARIOS: Dict[str, Dict[str, Any]] = {
    "ecommerce": {
        ...
        "nodes": [
            {"id": "client", "label": "Web & Mobile Client", "type": "gateway", "status": "HEALTHY", "position": {"x": 50, "y": 200}, "description": "Customer facing web storefront and mobile application"},
            {"id": "cdn", "label": "Cloud CDN", "type": "gateway", "status": "HEALTHY", "position": {"x": 250, "y": 200}, "description": "Edge routing & static asset caching"},
            {"id": "api_gateway", "label": "API Gateway", "type": "gateway", "status": "HEALTHY", "position": {"x": 450, "y": 200}, "description": "Central entrypoint routing traffic to internal microservices"},
            {"id": "auth_svc", "label": "Auth Service", "type": "service", "status": "HEALTHY", "position": {"x": 680, "y": 80}, "description": "Token verification and user session validation"},
            {"id": "order_svc", "label": "Order Service", "type": "service", "status": "HEALTHY", "position": {"x": 680, "y": 200}, "description": "Processes shopping carts and creates checkout orders"},
            {"id": "recommend_svc", "label": "Recommendation Engine", "type": "service", "status": "HEALTHY", "position": {"x": 680, "y": 320}, "description": "AI-powered product upsells (Soft dependency)"},
            {"id": "payment_api", "label": "Stripe Payment Gateway", "type": "third_party", "status": "HEALTHY", "position": {"x": 920, "y": 140}, "description": "External credit card & payment authorization", "backup_node_id": "backup_payment"},
            {"id": "backup_payment", "label": "PayPal Backup Gateway", "type": "third_party", "status": "HEALTHY", "position": {"x": 920, "y": 250}, "description": "Secondary standby payment gateway for failover"},
            {"id": "inventory_db", "label": "Inventory PostgreSQL", "type": "database", "status": "HEALTHY", "position": {"x": 920, "y": 360}, "description": "Primary database for product inventory & orders", "backup_node_id": "inventory_replica"},
            {"id": "inventory_replica", "label": "Inventory Standby Replica", "type": "database", "status": "HEALTHY", "position": {"x": 920, "y": 470}, "description": "Read-replica ready for hot failover"}
        ],
        "edges": [
    ...
    "microservices": {
        ...
        "nodes": [
            {"id": "load_balancer", "label": "Load Balancer (HAProxy)", "type": "gateway", "status": "HEALTHY", "position": {"x": 100, "y": 220}, "description": "Distributes incoming web traffic across services"},
            {"id": "user_svc", "label": "User Profile Service", "type": "service", "status": "HEALTHY", "position": {"x": 380, "y": 120}, "description": "Handles account info, authentication tokens"},
            {"id": "product_svc", "label": "Product Catalog Service", "type": "service", "status": "HEALTHY", "position": {"x": 380, "y": 320}, "description": "Fetches items, prices, and search results"},
            {"id": "redis_cache", "label": "Redis Cluster Cache", "type": "cache", "status": "HEALTHY", "position": {"x": 660, "y": 120}, "description": "In-memory caching (Soft dependency)"},
            {"id": "primary_db", "label": "Primary Master DB", "type": "database", "status": "HEALTHY", "position": {"x": 660, "y": 320}, "description": "Central relational database write cluster", "backup_node_id": "replica_db"},
            {"id": "replica_db", "label": "Standby Read Replica", "type": "database", "status": "HEALTHY", "position": {"x": 660, "y": 450}, "description": "Warm standby for database failover"},
            {"id": "analytics_queue", "label": "Kafka Event Queue", "type": "queue", "status": "HEALTHY", "position": {"x": 920, "y": 220}, "description": "Async telemetry stream (Soft dependency)"}
        ],
        "edges": [
    ...
    "banking": {
        ...
        "nodes": [
            {"id": "mobile_app", "label": "Mobile Banking Client", "type": "gateway", "status": "HEALTHY", "position": {"x": 80, "y": 200}, "description": "Customer mobile application frontend"},
            {"id": "secure_gateway", "label": "mTLS Security Gateway", "type": "gateway", "status": "HEALTHY", "position": {"x": 320, "y": 200}, "description": "Cryptographic authentication & firewall"},
            {"id": "fraud_svc", "label": "Real-time Fraud AI", "type": "service", "status": "HEALTHY", "position": {"x": 580, "y": 100}, "description": "Hard gate: transactions cannot execute without fraud signoff"},
            {"id": "transfer_svc", "label": "Wire Transfer Service", "type": "service", "status": "HEALTHY", "position": {"x": 580, "y": 300}, "description": "Core money movement orchestrator"},
            {"id": "core_ledger", "label": "Primary Core Ledger DB", "type": "database", "status": "HEALTHY", "position": {"x": 860, "y": 150}, "description": "Immutable bank ledger database", "backup_node_id": "backup_ledger"},
            {"id": "backup_ledger", "label": "Disaster Recovery Ledger", "type": "database", "status": "HEALTHY", "position": {"x": 860, "y": 290}, "description": "Synchronous replica in second data center"},
            {"id": "sms_notifier", "label": "SMS & Push Notifier", "type": "service", "status": "HEALTHY", "position": {"x": 860, "y": 420}, "description": "Customer transaction alerts (Soft dependency)"}
        ],
        "edges": [
...
def root():
    return {
        "app": "BreakPoint API",
        "status": "operational",
        "endpoints": ["/api/scenarios", "/api/simulate", "/api/recover"]
    }
...
def get_scenario(scenario_id: str):
    ...
    return DEFAULT_SCENARIOS[scenario_id]

@app.post("/api/simulate", response_model=SimulationResponse)
def simulate_failure(req: SimulationRequest):
    try:
        result = run_cascade_simulation(req.graph, req.failed_node_id)
        return result
...
def recover_failure(req: RecoveryRequest):
    try:
        result = run_recovery_simulation(req.graph, req.failed_node_id, req.strategy)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
